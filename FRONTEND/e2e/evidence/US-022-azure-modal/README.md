...
- **Mocks API (sem backend):** `FRONTEND/e2e/helpers/mock-api-for-config-pages.ts` — `installApiMocksForConfigIntegrationFlows` antes do login; utilizador simulado **Manager** quando o login usa `gestor.ti@g360.com.br` (email no POST); `admin@g360.com.br` → Super Admin global (outros specs).
