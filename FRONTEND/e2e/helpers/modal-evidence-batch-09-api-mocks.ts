...
/** Despesa `PREVISTO` — ícone «enviar» abre `SubmitExpenseModal`. */
const mockExpensePrevisto = {
    id: 'e2e-exp-prev-1',
    description: 'Despesa E2E submeter',
    status: 'PREVISTO',
    amount: 500,
    date: '2026-04-10T12:00:00.000Z',
    dueDate: '2026-04-25T00:00:00.000Z',
    supplier: { name: 'Fornecedor Mock' },
    costCenter: { code: 'CC-E2E', name: 'Centro E2E' },
    account: { name: 'Despesas Gerais' },
    invoiceNumber: null,
    fileUrl: null,
    approvalStatus: null,
...
/** Lista em `/suppliers` — visualização e edição na página de fornecedores. */
const mockSuppliersE2E = [
    {
        id: 'e2e-sup-v1',
        name: 'Fornecedor E2E Visual',
        tradeName: 'Fantasia E2E',
...
/**
 * API mock para `modal-evidence-batch-09.spec.ts` — SMTP, submeter despesa, fornecedores, tarefa geral (modais 41–45).
 * Baseado no lote 08 + integrações SMTP, despesas PREVISTO, lista de fornecedores, `POST /tasks`.
 */
export async function installModalEvidenceBatch09ApiMocks(page: Page): Promise<void> {
    await page.route('**/api/v1/**', async (route: Route) => {
        ...
        if (method === 'PUT' && /\/integrations\/SMTP\/?$/.test(path)) {
            await route.fulfill({
                status: 200,
                contentType: 'application/json',
                body: JSON.stringify({ type: 'SMTP', isEnabled: true, config: {} }),
            });
            return;
        }

        if (method === 'POST' && path.includes('/integrations/SMTP/test')) {
            await route.fulfill({
                status: 200,
                contentType: 'application/json',
                body: JSON.stringify({ message: 'Conexão SMTP OK (mock)' }),
            });
            return;
        }
        ...
        if (method === 'GET' && /\/suppliers\/?$/.test(path)) {
            await route.fulfill({
                status: 200,
                contentType: 'application/json',
                body: JSON.stringify(mockSuppliersE2E),
            });
            return;
        }

        if (method === 'POST' && /\/suppliers\/?$/.test(path)) {
            await route.fulfill({
                status: 201,
                contentType: 'application/json',
                body: JSON.stringify({ id: 'e2e-sup-new', name: 'Fornecedor novo E2E' }),
            });
            return;
        }
        ...
        if (method === 'GET' && /\/expenses\/?$/.test(path)) {
            await route.fulfill({
                status: 200,
                contentType: 'application/json',
                body: JSON.stringify([mockExpensePrevisto]),
            });
            return;
        }
        ...
        if (method === 'PUT' && /\/expenses\/[^/]+$/.test(path)) {
            await route.fulfill({
                status: 200,
                contentType: 'application/json',
                body: JSON.stringify({ ...mockExpensePrevisto, status: 'AGUARDANDO_APROVACAO' }),
            });
            return;
        }
        ...
        if (method === 'POST' && path.endsWith('/tasks') && !path.includes('/project-tasks')) {
            await route.fulfill({
                status: 201,
                contentType: 'application/json',
                body: JSON.stringify({ id: 'e2e-task-gen-1', title: 'Tarefa E2E' }),
            });
            return;
        }
        ...
        if (method === 'GET' && path.endsWith('/integrations')) {
            await route.fulfill({
                ...
                contentType: 'application/json',
                body: JSON.stringify([
                    { type: 'LDAP', isEnabled: false, config: null, lastSync: null },
                    {
                        type: 'SMTP',
                        isEnabled: false,
