...
- **GMUD — GMUD-02 (pós-conclusão, só leitura):** GMUD **finalizada** (`EXECUTED`, `FAILED`, `CANCELLED`) — **sem edição** no UI; **modal** em modo leitura (lista, deep link, detalhe com **Ver no** modal). `changeRequestViewUtils.js`. **Seed** `E2E-GMUD-GMUD02-EXEC-001` no fluxo 3 áreas. Backlog **GMUD-02** Corrigido.
  - `FRONTEND/src/utils/changeRequestViewUtils.js`, `ChangeRequestsPage.jsx`, `ChangeRequestDetailPage.jsx`, `GmudFormRouteRedirect.jsx`, `BACKEND` scripts de seed, `docs/backlog/melhorias-feedback-gestor.md`
...
### Added
- **Finanças / aprovações — DES-03 (despesas extra-orçamentárias):** `ApprovalTier.expensePlanScope` (`ALL` / `PLANNED` / `UNPLANNED`), script multi-tenant `npm run db:add-approval-tier-expense-plan-scope`, filtro de alçadas e notificações por âmbito; despesas `UNPLANNED` obrigam fluxo (`AGUARDANDO_APROVACAO`) e restrições de `status` na API; pendentes expõem `approvalStatus`; UI (aba Alçadas, lista/coluna âmbito, `MyApprovalsPage`, `ApprovalDetailsModal`, `ExpenseModal`, `ExpenseApprovalModal`). Testes em `approval-tier-expense-plan-scope.test.js` e extensões em testes de despesa. Backlog **DES-03** marcado Corrigido. `CLAUDE.md`: diretriz PostgreSQL multi-tenant.
  - `BACKEND/src/prisma/schema.prisma`, `BACKEND/src/scripts/add-approval-tier-expense-plan-scope-all-tenants.js`, `BACKEND/package.json`, `BACKEND/src/services/approval-tier.service.js`, `BACKEND/src/services/expense.service.js`, `BACKEND/src/controllers/expense.controller.js`, `BACKEND/src/controllers/approval.controller.js`, `BACKEND/src/validators/expense.validator.js`, `BACKEND/tests/...`
  - `FRONTEND/src/components/config/ApprovalTiersTab.jsx`, `approvalTierConstants.js`, `approvalTierListColumns.jsx`, `approvalTierListSort.js`, `FRONTEND/src/pages/approvals/MyApprovalsPage.jsx`, modais `ApprovalDetailsModal.jsx`, `ExpenseModal.jsx`, `ExpenseApprovalModal.jsx`
  - `docs/backlog/melhorias-feedback-gestor.md`, `CLAUDE.md`
...
- **Detalhe do contrato (`/contracts/:id`) — separador Anexos:** tabela MUI manual substituída por **`DataListTable`** com `contractAttachmentListColumns.jsx` e `contractAttachmentListSort.js`; ordenação por nome/data; `data-testid` **`tabela-contrato-anexos`**; toolbar de upload e permissões `CONTRACTS`/`ATTACH` inalteradas. Separador **Aditivos** permanece com **`DataListShell`** e lista em `<ul>`.
  ...
  - `docs/patterns/data-grids-inventory.md`, `docs/patterns/data-list-table-migration-plan.md`
- **Catálogo ITBM (`CatalogAdminPanel` em config Service Desk / rota legacy):** três separadores (**Serviços**, **Categorias**, **SLA**) passam de `Table` MUI manual para **`DataListTable`** (`catalogServiceList*`, `catalogCategoryList*`, `catalogSlaList*`); `data-testid` **`tabela-catalogo-servicos`**, **`tabela-catalogo-categorias`**, **`tabela-catalogo-slas`**. Modais e montador de formulário dinâmico inalterados.
  - `FRONTEND/src/pages/servicedesk/CatalogAdmin.jsx`, `FRONTEND/src/pages/servicedesk/catalogServiceListColumns.jsx`, `FRONTEND/src/pages/servicedesk/catalogServiceListSort.js`, `FRONTEND/src/pages/servicedesk/catalogCategoryListColumns.jsx`, `FRONTEND/src/pages/servicedesk/catalogCategoryListSort.js`, `FRONTEND/src/pages/servicedesk/catalogSlaListColumns.jsx`, `FRONTEND/src/pages/servicedesk/catalogSlaListSort.js`
  - `docs/patterns/data-grids-inventory.md`, `docs/patterns/data-list-table-migration-plan.md`
